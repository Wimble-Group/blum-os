...
const rotateSequenceImages = [];
const timestamp = '1668363658';



for (let i = 0; i <= 141; i ++) {
  appleSequenceImages.push(`${`0000${i}`.slice(-5)}.png?` + timestamp);
}
for (let i = 0; i <= 120; i ++) {
  rotateSequenceImages.push(`${`000${i}`.slice(-4)}.png?` + timestamp);
}
...
$(document).ready(function () {
  ...
  if ($(window).width() > 480) {
    ...
    const appleSequence = new ScrollSequence({
      ...
      imagesRoot: '/public/assets/img/home/glasses-sequel-compressed/',
      priorityFrames: [0, 20, 40, 90],
      cover: true,
    ...
    const rotateSequence = new ScrollSequence({
      container: '.tree-sequence',
      scrollWith: '.tree-container',
      images: rotateSequenceImages,
      imagesRoot: '/public/assets/img/home/glasses-rotate-compressed/',
...
function visibleBlock() {
  $('.speak-trigger').each(function() {
    var el = $(this),
        height = el.height() + 300;

    if (el[0].getBoundingClientRect().top == 0 || el[0].getBoundingClientRect().top <= 300 && el[0].getBoundingClientRect().top >= -150) {
      el.addClass('active');
    } else {
...
function showBackgroundRow()
{
  var el = $('.merge-worlds .right');
  
  if (el[0].getBoundingClientRect().top <= 10) {
    $('.merge-worlds').addClass('active')
  }
}
...
class Canvas {
  ...
  resize() {
    let w, h;
    if (this.container.matches('.tree-sequence')) {
      w = 622;
      h = 420;
    } else {
...
class ScrollSequence {
  ...
  changeOnWindowScroll() {
    const step = 50 / (this.images.length - 1);
...
function slickSlider()
{
  $("#slider").slick({
    ...
    customPaging: function(slider, i) { 
        return '<button class="tab" element=' +  $(slider.$slides[i]).attr('data-title') + '>' + $(slider.$slides[i]).attr('data-title');
    },
    arrows: false,
    infinite: true,
    speed: 0,
...
function transformSliderLine() 
{
  $('.slick-dots .tab').on('click', function() {
    $('.slick-dots').attr('el', $(this).attr('element'));
  })
}

function initVideo()
{
  $('.video-background video').append('<source src="/public/assets/video/gradient/gradient-compres.mp4?' + timestamp + '"' + 'type="video/mp4">');
}

function initSowLine()
{
  if (el[0].getBoundingClientRect().top >= 200 || el[0].getBoundingClientRect().top >= -200) {
    el.addClass('active');
  } else {
    el.removeClass('active');
  }
